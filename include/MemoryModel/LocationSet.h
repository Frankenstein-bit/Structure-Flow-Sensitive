...
public:
    ...
    /// Constructor
    LocationSet(s32_t o = 0 , s32_t m = 0) : fldIdx(o),fldOff(m)
    {}

    /// Copy Constructor
    LocationSet(const LocationSet& ls)
        : fldIdx(ls.fldIdx), offsetValues(ls.getOffsetValueVec()),fldOff(ls.getFldOff())
    {
    }
    ...
    inline s32_t getFldOff() const
    {
        return fldOff;
    }
    inline void setFldOff(s32_t off)
    {
        fldOff = off;
    }
    /// Dump location set
...
private:
    ...
    s32_t fldOff;    // Accumulated memory Offsets flat
};
