...
public:
    ...
    {
        //std::cout << "getDFOutPtsSet:"<< loc <<"   loc:"<< var  <<std::endl;
    ...
    {
        //std::cout << "updateAllDFInFromOut***34:---"<< srcLoc << "  o:"<< srcVar<<std::endl;
...
public:
    ...
    virtual inline bool updateDFInFromIn(LocID srcLoc, const Key& srcVar, LocID dstLoc, const Key& dstVar) override
    {
        //std::cout << "***35:www"<< dstLoc << "  o:"<< dstVar<<std::endl;
    ...
    virtual inline bool updateTLVPts(LocID srcLoc, const Key& srcVar, const Key& dstVar) override
    {
        //std::cout << "***34:"<< srcLoc <<"   srcVar:"<< srcVar <<std::endl;
        if(varHasNewDFInPts(srcLoc,srcVar))
        {
            //std::cout << "333falg" <<std::endl;
            removeVarFromDFInUpdatedSet(srcLoc,srcVar);
    ...
    virtual inline bool updateATVPtsFromPts(const DataSet& srcDataSet, LocID dstLoc, const Key& dstVar)
    {

        if (this->unionPts(this->getDFOutPtsSet(dstLoc, dstVar), srcDataSet))
        {
            setVarDFOutSetUpdated(dstLoc, dstVar);
            return true;
        }
    ...
    virtual inline bool varHasNewDFInPts(LocID loc,const Key& var)
    {
    ...
    virtual inline bool varHasNewDFOutPts(LocID loc,const Key& var)
    {
...
private:
    ...
    {
        //std::cout << "setVarDFInSetUpdated" <<std::endl;
    ...
    {
        //std::cout << "getDFInUpdatedVar" <<std::endl;
