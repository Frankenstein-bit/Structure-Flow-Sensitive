...
protected:
    ...
    inline bool isVarHasNewDFInPts(NodeID srcId, NodeID srcVar){ 
        return getDFPTDataTy()->varHasNewDFInPts(srcId,srcVar);
    }
    inline bool isVarHasNewDFOutPts(NodeID srcId, NodeID srcVar){
        return getDFPTDataTy()->varHasNewDFOutPts(srcId,srcVar);
    }
    ...
    virtual inline bool unionPtsFromTopPts(const SVFGNode* stmt, NodeID dstVar ,const PointsTo& target)
    {
        //SVFUtil::outs() << "here"  <<"\n";
        return getDFPTDataTy()->updateATVPtsFromPts(target,stmt->getId(),dstVar);
    }
    ...
    //those are for struct-sensitive
    //这里类型不对，还需要修改
    // virtual bool processSubtype(const CopySVFGNode* copy) ;
    // virtual bool processConstruct(const CopySVFGNode* copy) ;
    // virtual bool processDynamic(const CopySVFGNode* copy) ;
    ...
    virtual void countAliases(Set<std::pair<NodeID, NodeID>> cmp, unsigned *mayAliases, unsigned *noAliases,unsigned *oherCase, unsigned *queries);
    ...
    virtual void printCTirAliasStats(void);
        ...
        int bitCastinstuct = 0;
    int bitCastinstuctWork = 0;
    
        map<NodeID,map<int,int>> typeMap;
    map<NodeID,map<int, map<int,int>>> typeSubMap;
    //set<const Instruction*> castInstSet;
