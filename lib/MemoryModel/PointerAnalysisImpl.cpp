    ...
    PointerAnalysis(p, type, alias_check), ptCache()
{
    ...
    else if (type == FSSPARSE_WPA)
    {
        if (Options::INCDFPTData)
        {
            ...
            //SVFUtil::outs() << "firstType--:wwwwwwwwwwwwww" << " "  <<"\n";
            if (Options::ptDataBacking == PTBackingType::Mutable) {/* SVFUtil::outs() << "firstType--:aaaaaa" << " "  <<"\n";*/ ptD = new MutIncDFPTDataTy(false);}
            else if (Options::ptDataBacking == PTBackingType::Persistent) {/*SVFUtil::outs() << "firstType--:22222" << " "  <<"\n";*/ptD = new PersIncDFPTDataTy(getPtCache(), false);}
            else assert(false && "BVDataPTAImpl::BVDataPTAImpl: unexpected points-to backing type!");
        }
        else
        {
            // SVFUtil::outs() << "firstType--:zzzzz" << " "  <<"\n";
 ...
 */
void BVDataPTAImpl::onTheFlyCallGraphSolve(const CallSiteToFunPtrMap& callsites, CallEdgeMap& newEdges, bool virFlag)
{
    ...
    for(CallSiteToFunPtrMap::const_iterator iter = callsites.begin(), eiter = callsites.end(); iter!=eiter; ++iter)
    {
        ...
        if (isVirtualCallSite(SVFUtil::getLLVMCallSite(cs->getCallSite())))
        {
            if(!virFlag){
                ...
                //two condition to choose different path
                //just set a bool.
                //here we get the object load from where,
                //which means that we need to modify "getVCallVtblPtr"
                //here actually get the vptr
                // const Value *vptr = getVCallVptr(SVFUtil::getLLVMCallSite(cs->getCallSite()));
                
                // //add check here to see if this make the result bad.
                // std::string str1;
                // llvm::raw_string_ostream rso1(str1);
                // vptr->print(rso1);
                // rso1.flush();
                // std::cout << "onTheFlyCallGraphSolve1:" <<str1 << std::endl;

                //the problem is can we get object?
                //here we should get the vtable? or the object , need to consider.
                //if object we need to get offset,the offset need to computer by ourself. but it is easy to get.
                //we may need to rewrite from here,or from below
                //
                // std::string str1;
                // llvm::raw_string_ostream rso1(str1);
                // vtbl->print(rso1);
                // rso1.flush();
                // std::cout << "www:" <<str1 << std::endl;
                //here is the thing,we consider.
                //assert(pag->hasValueNode(vptr));
                //NodeID vptrId = pag->getValueNode(vptr);
                resolveVirCallbyType(cs, newEdges);
            }
