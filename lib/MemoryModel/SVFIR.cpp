...
NodeID SVFIR::getGepObjVar(NodeID id, const LocationSet& ls)
{
    ...
    //std::cout << "***34:"<<node->toString() <<std::endl;
    // std::cout << "objectoff:"<<ls.getFldOff() << endl;
    // std::cout << "fldidx:"<<ls.accumulateConstantFieldIdx() << endl;
    if (GepObjVar* gepNode = SVFUtil::dyn_cast<GepObjVar>(node))
    {
        //std::cout << "***33:"<<gepNode->toString() << endl;
        //here the gepNode->getLocationSet() + ls to pass can be seen as new ls
        // std::cout << "gepoff:"<<gepNode->getLocationSet().getFldOff() << endl;
        // std::cout << "objectoff:"<<ls.getFldOff() << endl;
...
NodeID SVFIR::getGepObjVar(const MemObj* obj, const LocationSet& ls)
{
    ...
    //std::cout << "ooo---12" << std::endl;
    LocationSet newLS = SymbolTableInfo::SymbolInfo()->getModulusOffset(obj,ls);
    newLS.setFldOff(ls.getFldOff());
    // std::cout << "ooo---12:"<< newLS.accumulateConstantFieldIdx()  << std::endl;
    //std::cout << "off-13:"<< newLS.getFldOff()  << std::endl;
    ...
    if (Options::FirstFieldEqBase && newLS.getFldOff() == 0) return base;

    NodeLocationSetMap::iterator iter = GepObjVarMap.find(std::make_pair(base, newLS));
    //std::cout << "targetID:"<< newLS.getFldOff()  << std::endl;
...
NodeID SVFIR::addGepObjNode(const MemObj* obj, const LocationSet& ls)
{
    ...
    //std::cout << "ooo---12:-------:"<< gepId << std::endl;
