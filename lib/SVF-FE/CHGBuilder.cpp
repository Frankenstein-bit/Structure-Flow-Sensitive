...
void CHGBuilder::connectInheritEdgeViaCall(const SVFFunction* callerfun, CallSite cs)
{
    ...
    //std::cout << "dname:"<<dname.className << "  "<<  dname.funcName << std::endl;
    if ((isConstructor(caller) && isConstructor(callee)) || (isDestructor(caller) && isDestructor(callee)))
    {
        //std::cout << "connectInheritEdgeViaCall:true"  << std::endl;
...
void CHGBuilder::analyzeVTables(const Module &M)
{
            ...
            E = M.global_end(); I != E; ++I)
    {
        ...
        if (isValVtbl(globalvalue) && globalvalue->getNumOperands() > 0)
        {
            ...
            // globalvalue->print(rso);
            // rso.flush();
            // SVFUtil::outs() <<",,," <<str << std::endl;
            ...
            // vtblStruct->print(rso1);
            // rso1.flush();
            // std::cout << "-----" <<str1 << std::endl;

            string vtblClassName = getClassNameFromVtblObj(globalvalue);
            //SVFUtil::outs() << "vtblClassName))):"<< vtblClassName   <<"\n";
            ...
            for (unsigned int ei = 0; ei < vtblStruct->getNumOperands(); ++ei)
            {
                ...
                while (i < vtbl->getNumOperands())
                {
                    ...
                    int null_ptr_num = 0;
                    // here are used to skip null
                    for (; i < vtbl->getNumOperands(); ++i)
                    {

                        // std::string str3;
                        // llvm::raw_string_ostream rso3(str3);
                        // vtbl->getOperand(i)->print(rso3);
                        // rso3.flush();
                        // std::cout << "$$$$$" <<str3 << std::endl;

                        if (SVFUtil::isa<ConstantPointerNull>(vtbl->getOperand(i)))
                        {
                            //check if before one is null?
                            if (i > 0 && !SVFUtil::isa<ConstantPointerNull>(vtbl->getOperand(i-1)))
                            {

                                const ConstantExpr *ce =
                                    SVFUtil::dyn_cast<ConstantExpr>(vtbl->getOperand(i-1)); //get before one 
                                //check if the before one is bitcast?
                                if (ce->getOpcode() == Instruction::BitCast)
                                {
                                    const Value *bitcastValue = ce->getOperand(0);
                        ...
                        if (opcode == Instruction::BitCast)
                        {
                            ...
                            if (const Function* f = SVFUtil::dyn_cast<Function>(bitcastValue))
                            {
                                ...
                                //std::cout << "dname:" << dname.funcName << std::endl;
                    ...
                    //add a function to set the map between the class and the virtualFun
                    //and consider about the map between the vptr and virFun
                    // for(auto vv: virtualFunctions){
...
void CHGBuilder::analyzeVTables(const Module &M, std::set<std::string> &classNameSet)
{
    for (Module::const_global_iterator I = M.global_begin(),
            E = M.global_end(); I != E; ++I)
    {
        ...
        if (isValVtbl(globalvalue) && globalvalue->getNumOperands() > 0)
        {
            ...
            for (unsigned int ei = 0; ei < vtblStruct->getNumOperands(); ++ei)
            {
                 ...
                 * 3. i8* bitcast xxx
                 */
                //here the vtable map to the vptr according to set.
                //we can get the offset from here or somewhere else.
                bool pure_abstract = true;
                u32_t i = 0;
                while (i < vtbl->getNumOperands())
                {
