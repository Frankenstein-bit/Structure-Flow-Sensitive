...
bool cppUtil::isVirtualCallSite(CallSite cs)
{
    ...
    if (const LoadInst *vfuncloadinst = SVFUtil::dyn_cast<LoadInst>(vfunc))
    {
                    ...
                    SVFUtil::dyn_cast<GetElementPtrInst>(vfuncptr))
        {
            ...
            // std::string str3;
            // llvm::raw_string_ostream rso3(str3);
            // vtbl->print(rso3);
            // rso3.flush();
 ...
 */
// %vtable = load vptr
//vptr is part of object.we use this value to get the object
//add the getPointerOperand will be enough to get the object,and we can analysis to check the offset, 
//and check if target object.
...
//and check if target object.
/*
  %20 = bitcast %struct.Right* %19 to void (%struct.Right*)***
  %21 = load void (%struct.Right*)**, void (%struct.Right*)*** %20, align 8
  %22 = getelementptr inbounds void (%struct.Right*)*, void (%struct.Right*)** %21, i64 2
  %23 = load void (%struct.Right*)*, void (%struct.Right*)** %22, align 8
  call void %23(%struct.Right* nonnull align 8 dereferenceable(8) %19)
*/
const Value *cppUtil::getVCallVptr(CallSite cs)
{
    const LoadInst *loadInst = SVFUtil::dyn_cast<LoadInst>(cs.getCalledValue());
    ...
    // rso4.flush();
    // std::cout << "www44:" <<str4 << std::endl;
    const LoadInst *loadInstVt = SVFUtil::dyn_cast<LoadInst>(vtbl);
    assert(loadInstVt != nullptr);
    const Value *vptrFun = loadInstVt->getPointerOperand();

    std::string str4;
    llvm::raw_string_ostream rso4(str4);
    vptrFun->print(rso4);
    rso4.flush();
    std::cout << "www55:" <<str4 << std::endl;
    
    const CastInst* castInst = llvm::dyn_cast<CastInst>(vptrFun);
    assert(castInst != nullptr);
    const Value *vptr = castInst->getOperand(0);

    std::string str5;
    llvm::raw_string_ostream rso5(str5);
    vptr->print(rso5);
    rso5.flush();
    std::cout << "www66:" <<str5 << std::endl;
...
string cppUtil::getClassNameFromVtblObj(const Value *value)
{
    ...
    //SVFUtil::outs() << "vtblName))):"<< vtblName   <<"\n";
    s32_t status;
    char *realname = abi::__cxa_demangle(vtblName.c_str(), 0, 0, &status);
    if (realname != nullptr)
    {
        string realnameStr = string(realname);
        //SVFUtil::outs() << "realnameStr))):"<< realnameStr   <<"\n";
...
bool cppUtil::isConstructor(const Function *F)
{
    ...
    //std::cout << "isConstructor:" << funcName << std::endl;
...
string cppUtil::getClassNameOfThisPtr(CallSite cs)
{
    ...
    //std::cout << "thisPtrClassName:" << thisPtrClassName << std::endl;
