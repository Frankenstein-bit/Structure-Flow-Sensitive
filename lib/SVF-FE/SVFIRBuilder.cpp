...
bool SVFIRBuilder::computeGepOffset(const User *V, LocationSet& ls)
{
            ...
            gi != ge; ++gi)
    {
        ...
        // gepTy->print(rso2);
        // rso2.flush();
        // std::cout << "&&&&&9" <<str2 << std::endl;

        const Value* offsetVal = gi.getOperand();

        // std::string str3;
        // llvm::raw_string_ostream rso3(str3);
        // offsetVal->print(rso3);
        // rso3.flush();
        // std::cout << "&&&&&109" <<str3 << std::endl;
...
void SVFIRBuilder::visitGetElementPtrInst(GetElementPtrInst &inst)
{
    ...
    // inst.print(rso1);
    // rso1.flush();
    // std::cout << "@@@@----2" <<str1 << std::endl;

    // std::string str2;
    // llvm::raw_string_ostream rso2(str2);
    // inst.getPointerOperand()->print(rso2);
    // rso2.flush();
    // std::cout << "@@@@dst----3" <<str2 << std::endl;
    ...
    // s32_t offset = ls.accumulateConstantOffset();
    // std::cout << "@@@@----90：" <<offset << std::endl;
    // std::cout << "fldIdx----95：" <<ls.accumulateConstantFieldIdx() << std::endl;
    // s32_t memOff = ls.getFldOff();
    // std::cout << "@@@@****91：" << memOff << std::endl;
    //std::cout << "----)"  << std::endl;
...
{
    //SVFUtil::outs() << "SVFIRBuilder:addComplexConsForExt"<< "\n";
    assert(D && S);
            ...
            if (item != nullptr)
            {
                ...
                while (obj)
                {
                    ...
                    case ExtAPI::EXT_COPY_N:
                    {
                        ...
                        for (u32_t index = 0; index < sz; index++)
                        {
                            //SVFUtil::outs() << "SVFIRBuilder:handleExtCall"<< "\n";
