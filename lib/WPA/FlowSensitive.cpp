...
void FlowSensitive::processNode(NodeID nodeId)
{
    ...
    //SVFUtil::outs() << "processNode111--:"<< node->toString()  <<"\n";
    if (processSVFGNode(node))
        propagate(&node);
    //SVFUtil::outs() << "processNode--:"<< node->toString()  <<"\n";
...
{
    //SVFUtil::outs() << "processSVFGNode14" << "\n";
    double start = stat->getClk();
    bool changed = false;
    if(AddrSVFGNode* addr = SVFUtil::dyn_cast<AddrSVFGNode>(node))
    {
        //SVFUtil::outs() << "processSVFGNode15" << "\n";
        numOfProcessedAddr++;
        if(processAddr(addr))
            changed = true;
        //SVFUtil::outs() << "processSVFGNode15-" << "\n";
    ...
    {
        //SVFUtil::outs() << "processSVFGNode16" << "\n";
        numOfProcessedCopy++;
        if(processCopy(copy))
            changed = true;
        //SVFUtil::outs() << "processSVFGNode16-" << "\n";
    ...
    {
        //SVFUtil::outs() << "processSVFGNode17" << "\n";
        numOfProcessedGep++;
        if(processGep(gep))
            changed = true;
        //SVFUtil::outs() << "processGep-" << "\n";
    }
    else if(LoadSVFGNode* load = SVFUtil::dyn_cast<LoadSVFGNode>(node))
    {
        //SVFUtil::outs() << "processSVFGNode18" << "\n";
        numOfProcessedLoad++;
        if(processLoad(load))
            changed = true;
        //SVFUtil::outs() << "processSVFGNode18-" << "\n";
    ...
    {
        //SVFUtil::outs() << "processSVFGNode19" << "\n";
        numOfProcessedStore++;
    ...
    {
        //SVFUtil::outs() << "processSVFGNode20" << "\n";
        numOfProcessedPhi++;
        if (processPhi(phi))
            changed = true;
        //SVFUtil::outs() << "processSVFGNode20-" << "\n";
    ...
    {
        //SVFUtil::outs() << "processSVFGNode21" << "\n";
...
// propFromSrcToDst do not need to overwirte
// here it only analysize edge type the real Propagate are in 
// propAlongDirectEdge() and propAlongIndirectEdge()
bool FlowSensitive::propFromSrcToDst(SVFGEdge* edge)
{
    //SVFUtil::outs() << "propFromSrcToDst--:"<< "\n";
    double start = stat->getClk();
    bool changed = false;
...
bool FlowSensitive::processAddr(const AddrSVFGNode* addr)
{
    ...
    // SVFUtil::outs() << "processAddr))):"<< addr->toString()   <<"\n";
    // SVFUtil::outs() << "NodeID:"<< addr->getId()   <<"\n";
    // SVFUtil::outs() << "srcID:"<< srcID   <<"\n";
    // SVFUtil::outs() << "dstID:"<< addr->getPAGDstNodeID()   <<"\n";
...
{
    double start = stat->getClk();
    // SVFUtil::outs() << "processCopy))):"<< copy->toString()   <<"\n";
    // SVFUtil::outs() << "NodeID:"<< copy->getId()   <<"\n";
    // SVFUtil::outs() << "srcID:"<< copy->getPAGSrcNodeID()   <<"\n";
    // SVFUtil::outs() << "dstID:"<< copy->getPAGDstNodeID()   <<"\n";
    //SVFUtil::outs() << "processAddr))):"<< copy->toString()   <<"\n";
    // const PointsTo& srcPts = getPts(copy->getPAGSrcNodeID());
    ...
    //         for(NodeID o : srcPts){ 
    //             //std::cout << "bitcast-1:"<< o << std::endl;
    //             bitCastObj = bitCastObj + 1;
    //         }
    //     }
    // }

    //SVFUtil::outs() << "processAddr---------:"<< getPAG()->getGNode(copy->getPAGDstNodeID())->toString() << "-------"<< getPAG()->getGNode(copy->getPAGSrcNodeID())->toString()  <<"\n";
...
bool FlowSensitive::processGep(const GepSVFGNode* edge)
{
    ...
    //SVFUtil::outs() << "processGep))):"<< edge->toString()   <<"\n";
    // SVFUtil::outs() << "NodeID:"<< edge->getId()   <<"\n";
    // SVFUtil::outs() << "srcID:"<< edge->getPAGSrcNodeID()  <<"\n";
    // SVFUtil::outs() << "dstID:"<< edge->getPAGDstNodeID()   <<"\n";
...
bool FlowSensitive::updateCallGraph(const CallSiteToFunPtrMap& callsites)
{
    ...
    SVFUtil::outs() << "updateCallGraph:222"  << "\n";
    onTheFlyCallGraphSolve(callsites, newEdges);
    SVFUtil::outs() << "updateCallGraph:"  << "\n";
...
void FlowSensitive::printCTirAliasStats(void)
{
    ...
    // std::cout<<"memObj:"<< getPAG()->getObjectNodeNum() <<"\n";
    // std::cout<<"FieldValNodeNum:"<< getPAG()->getFieldValNodeNum() <<"\n";
    // std::cout<<"getFieldObjNodeNum:"<< getPAG()->getFieldValNodeNum() <<"\n";
    // std::cout<<"copyNum:"<< getPAG()->getFieldValNodeNum() <<"\n";
    // < SVFG node ID (loc), PAG node of interest (top-level pointer) >.
    std::cout<<"printCTirAliasStats" <<"\n";
    ...
    unsigned mayAliases = 0, noAliases = 0 , oherCase = 0;
    unsigned queries = 0;
    countAliases(cmpLocs, &mayAliases, &noAliases, &oherCase ,&queries);

    unsigned total = mayAliases + noAliases + oherCase;
    llvm::outs() << "total queries: "
                 << queries << " "
                 << "\n";
    ...
    llvm::outs() << "  " << "TOTAL : " << total << "\n"
                 << "  " << "MAY   : " << mayAliases << "\n"
                 << "  " << "MAY % : " <<  round(((double)mayAliases/(double)(total))*10000.0) /100.0 << "\n"
                 << "  " << "NO    : " << noAliases << "\n"
                 << "  " << "NO  % : " <<  round(((double)noAliases/(double)(total))*10000.0)/100.0 << "\n";
    std::cout<<"printCTirAliasStats---------------" <<"\n";
}


void FlowSensitive::countAliases(Set<std::pair<NodeID, NodeID>> cmp, unsigned *mayAliases, unsigned *noAliases ,unsigned *oherCase, unsigned *queries)
{
