...
void WPAPass::runOnModule(SVFModule* svfModule, bool structFlag)
{

    runPointerAnalysis(svfModule, 0 , structFlag);
 ...
 */
void WPAPass::runPointerAnalysis(SVFModule* svfModule, u32_t kind, bool structFlag)
{
    /// Build SVFIR
    ...
    if(structFlag){
        _pta = new FlowSensitiveStruct(pag);
